...
class APK :
    ...
    def __init__(self, filename, raw=False, domd5=True) :
        """
        ...
        if domd5:
#self.file_md5 = util.get_md5(self.__raw)
            pass
        ...
        reCert = re.compile(r"meta-inf(/|\\).*\.(rsa|dsa)")
        reSF = re.compile(r"meta-inf(/|\\).*\.sf")

        for i in self.zip.namelist() :
            if i == "AndroidManifest.xml" :
                decoded = AXMLPrinter( self.zip.read( i ) ).getBuff()
                self.xml[i] = minidom.parseString(decoded  )

                self.package = self.xml[i].documentElement.getAttribute( "package" )
                self.label = self.xml[i].documentElement.getAttribute( "package" )
            ...
            if reCert.match(i.lower()):
                self.parse_cert (i)

            if reSF.match(i.lower()):
                pass
#self.sf_md5 = util.get_md5( self.zip.read(i) )

#if fnmatch.fnmatch(i.lower(), "meta-inf/manifest.mf"):
            if i.lower() == "meta-inf/manifest.mf":
                self.parse_mf ( self.zip.read(i) )

    def parse_mf (self, mf_raw):
#self.mf_md5 = util.get_md5(mf_raw)

        CURRENT_IS_OTHER, CURRENT_IS_AXML, CURRENT_IS_DEX, CURRENT_IS_ARSC = range(4)
        current_file = CURRENT_IS_OTHER

        for line in mf_raw.splitlines():
            kv = line.rstrip().split(":")
            if len(kv) < 2:
                continue

            if current_file == CURRENT_IS_AXML:
                self.axml_digest_method = kv[0].strip()
                self.axml_digest = kv[1].strip()
                current_file = CURRENT_IS_OTHER
                continue
            elif current_file == CURRENT_IS_DEX:
                self.dex_digest_method = kv[0].strip()
                self.dex_digest = kv[1].strip()
                current_file = CURRENT_IS_OTHER
                continue
            elif current_file == CURRENT_IS_ARSC:
                self.arsc_digest_method = kv[0].strip()
                self.arsc_digest = kv[1].strip()
                current_file = CURRENT_IS_OTHER
                continue
            else:
                pass

            if kv[0].lower().strip() == "name":
                fname = kv[1].lower().strip()
                if fname == "classes.dex":
                    current_file = CURRENT_IS_DEX
                elif fname == "resources.arsc":
                    current_file = CURRENT_IS_ARSC
                elif fname == "androidmanifest.xml":
                    current_file = CURRENT_IS_AXML
                else:
                    current_file = CURRENT_IS_OTHER
    ...
    def parse_cert(self, cert_fname):
        ...
        cert = sk3.pop()
        md5 = hashlib.md5(cert.as_der()).hexdigest()  
        print md5
#self.cert_text = cert.as_text()
#self.cert_md5 = util.get_md5(cert.as_der())
    ...
    def get_permissions(self) :
        """
            Return permissions
        """
        return [x.rsplit('.', 1)[-1] for x in self.permissions]
    ...
    def get_intentfilters(self):
        """
            Return the android:name attributes for <action> and <category>
            plus <data> with:
            android:scheme
            android:host
            android:port
        ...
        if hasattr(self, "intentfilters"):
            return self.intentfilters

        self.intentfilters = []
        self.intentfilters.extend( "action=" + x for x in (list(set(self.get_elements("action", "android:name") )))  )
        self.intentfilters.extend( "category=" + x for x in (list(set(self.get_elements("category", "android:name") ))) )
        self.intentfilters.extend( "data_scheme=" + x for x in (list(set(self.get_elements("data", "android:scheme") ))) )
        self.intentfilters.extend( "data_host=" + x for x in (list(set(self.get_elements("data", "android:host") ))) )
        self.intentfilters.extend( "data_port=" + x for x in (list(set(self.get_elements("data", "android:port") ))) )
        self.intentfilters.extend( "data_path=" + x for x in (list(set(self.get_elements("data", "android:path") ))) )
        self.intentfilters.extend( "data_pathPattern=" + x for x in (list(set(self.get_elements("data", "android:pathPattern") ))) )
        self.intentfilters.extend( "data_pathPrefix=" + x for x in (list(set(self.get_elements("data", "android:pathPrefix") ))) )
        self.intentfilters.extend( "data_mimeType=" + x for x in (list(set(self.get_elements("data", "android:mimeType") ))) )
        return self.intentfilters
...
#           print "tagtype = ", _type
            ...
            if _type == START_DOCUMENT :
                ...
                for i in range(0, self.axml.getAttributeCount()) :
                    prefix = self.getPrefix( self.axml.getAttributePrefix(i) ) or ""
                    attr_name = self.axml.getAttributeName(i);
                    attr_value = self.getAttributeValue( i ) \
                                         ...
                                         .replace("'", "&apos;") \
                                         .replace("\"", "%quot;")
                    if len(prefix) == 0 or prefix == "android:":
...
#apkf = APK(u"Y:\\yunos_virus\\168彩票官方版-1.apk")
    apkf = APK('/home/xiangxiaowei/share/weibo_1083.apk')
