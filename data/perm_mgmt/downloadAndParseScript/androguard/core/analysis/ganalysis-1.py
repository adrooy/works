...
class GVMAnalysis:
    def __init__(self, vmx, apk):
      ...
      #      sys.exit()
        ...
        '''
        permissionFile = os.path.join(os.getcwd(), 'PERMISSIONS')
        permissions = {}
...
#                print src_class_name, src_method_name, src_descriptor
#                print dst_class_name, dst_method_name, dst_descriptor
                #print '\n'
#                print x, dst_class_name, dst_method_name, dst_descriptor, 'dst'
#                print x, src_class_name, src_method_name, src_descriptor, 'src'
#                import sys
#                sys.exit()

                dst_line = '%s->%s%s' % (dst_class_name, dst_method_name, dst_descriptor)
                src_line = '%s->%s%s' % (src_class_name, src_method_name, src_descriptor)
                
                line = '%s,%s' % (dst_line, src_line)

                lbe_permission = permissions.get(x) if x in permissions else x

                if line not in self.process:
                    if x in permissions:
                        lbe_permission = permissions.get(x)
                        if lbe_permission in self.result:
                            permission_dict = self.result[lbe_permission]
                            if x in permission_dict:
                                permission_dict[x].append(line)
                            else:
                                permission_dict[x] = [line]
                        else:
                            permission_dict = {}
                            permission_dict[x] = [line]
                            self.result[lbe_permission] = permission_dict
                    else:
                        lbe_permission = x
                        if lbe_permission in self.otherresult:
                            permission_dict = self.otherresult[lbe_permission]
                            if x in permission_dict:
                                permission_dict[x].append(line)
                            else:
                                permission_dict[x] = [line]
                            ...
                            permission_dict = {}
                            permission_dict[x] = [line]
                            self.otherresult[lbe_permission] = permission_dict

                self.result.setdefault(line, x)
                import sys
                sys.exit()
                '''
    ...
    def get_and_permissions(self):
        return self.and_permissions

    def get_result(self):
        return self.result

    def get_otherresult(self):
        return self.otherresult
